...
package org.wisepersist.appengine.guestbook;
...
import org.wisepersist.appengine.guestbook.SignGuestbookServlet;
...
public class SignGuestbookServletTest {
  ...
  public void testDoPost() throws IOException, EntityNotFoundException {
    ...
    assertEquals(greeting.getKey().getParent().getName(), guestbookName);
    assertEquals(greeting.getProperty("content"), testContent);
    assertEquals(greeting.getProperty("user"), currentUser);

    Date date = (Date) greeting.getProperty("date");
    assertTrue(priorToRequest.before(date) || priorToRequest.equals(date),
               "The date in the entity [" + date + "] is prior to the request being performed");
    assertTrue(afterRequest.after(date) || afterRequest.equals(date),
